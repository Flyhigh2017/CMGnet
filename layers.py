...
def conv_btn(inputs, kernel_size, num_outputs, name,
             is_training=True, dilations=None, stride_size=[1, 1], padding='SAME', activation_fn=tf.nn.relu):
    """
    ...
    with tf.variable_scope(name):
        ...
        if dilations is not None:
            ...
            conv = tf.nn.atrous_conv2d(inputs, weights, dilations, padding=padding)
        else:
            conv = tf.nn.conv2d(inputs, weights, stride_shape, padding=padding)
        outputs = tf.nn.bias_add(conv, bias)
...
def concat(inputs1, inputs2, name, axis=3):
    """
    ...
    return tf.concat(axis=axis, values=[inputs1, inputs2], name=name)
